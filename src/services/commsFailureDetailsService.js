...
;(function(FailureType) {
  ...
  FailureType["THREADS_CONTROLLABLE"] = "THREADS_CONTROLLABLE"
  FailureType["THREADS_NON_CONTROLLABLE"] = "THREADS_NON_CONTROLLABLE"
...
export async function fetchFailureDetails(failureType, filters) {
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      let mockData = generateMockFailureDetails(failureType)

      // For controllable/non-controllable types, filter the results using the same categorization logic
      if (failureType === FailureType.THREADS_CONTROLLABLE) {
        mockData = mockData.filter(
          item => categorizeErrorMessage(item.errorMessage) === "CONTROLLABLE"
        )
      } else if (failureType === FailureType.THREADS_NON_CONTROLLABLE) {
        mockData = mockData.filter(
          item =>
            categorizeErrorMessage(item.errorMessage) === "NON_CONTROLLABLE"
        )
      }
    ...
    if (failureType === FailureType.THREADS_CONTROLLABLE) {
      ...
      return processedData.filter(
        item => categorizeErrorMessage(item.errorMessage) === "NON_CONTROLLABLE"
      )
    }
...
// Function to fetch combined API and delivery failures for categorization using a single query
export async function fetchCombinedFailureDetailsForCategorization(filters) {
  try {
    // For development/testing, use mock data
    if (process.env.NODE_ENV === "development") {
      console.log("Using mock data for combined failure categorization")

      // Generate mock data that includes both API and delivery failures
      const mockApiFailures = generateMockFailureDetails(
        FailureType.THREADS_API
      )
      const mockDeliveryFailures = generateMockFailureDetails(
        FailureType.THREADS_DELIVERY
      )

      // Combine both types
      const combinedMockData = [...mockApiFailures, ...mockDeliveryFailures]
      console.log(
        "Generated combined mock data for categorization:",
        combinedMockData.length,
        "error types"
      )
      return combinedMockData
    }
    ...
    if (filters?.startDate && filters?.endDate) {
      ...
      const startTimestamp = dateTimeToEpoch(filters.startDate, startTime)
      const endTimestamp = dateTimeToEpoch(filters.endDate, endTime)

      dateFilter = `toUnixTimestamp(eventTimestamp) >= ${startTimestamp} AND toUnixTimestamp(eventTimestamp) <= ${endTimestamp}`
    } else {
      // Use current month as default
      const { startDate, endDate } = getCurrentMonthRange()
      const { startTime, endTime } = getDefaultTimeRange()

      const startTimestamp = dateTimeToEpoch(startDate, startTime)
      const endTimestamp = dateTimeToEpoch(endDate, endTime)

      dateFilter = `toUnixTimestamp(eventTimestamp) >= ${startTimestamp} AND toUnixTimestamp(eventTimestamp) <= ${endTimestamp}`
    }

    // Build tenant and dealer filters
    // Build the combined query for both API and delivery failures
    // FIXED: Remove LIMIT to get all data (was truncating at 1000 records)
    const query = `
    SELECT
      eventMessage,
      CONCAT(JSONExtractString(eventMetaData, 'errorMessage'), ' ', JSONExtractString(eventMetaData, 'deliveryStatusDescription')) as errorMessage,
    ...
    WHERE
      ...
      ${filters?.dealerId ? `AND dealerId = '${filters.dealerId}'` : ""}
    GROUP BY
      eventMessage, errorMessage, origin
    ORDER BY
      count DESC;
    ...
    console.log("Combined Failure Categorization API Request:", payload)
    ...
    console.log("Combined Failure Categorization API Response:", response.data)

    // Process the raw data into our structured format
    const processedData = processRawFailureDetails(response.data.data.data)

    console.log(
      "Processed combined failure data for categorization:",
    ...
    console.error(
      "Error fetching combined failure details for categorization:",
      error
    )
    throw error
  }
...
function processRawSpecificTenantDealerMetrics(rawData, tenantId, dealerId) {
  ...
  return {
    ...
    threadsControllableError: 0, // TODO: Calculate from actual data
    threadsNonControllableError: 0, // TODO: Calculate from actual data
    threadsControllableErrorRate: "0.0%",
    threadsNonControllableErrorRate: "0.0%",
    threadsFailureRate,
    // GTC - CRM Non-Threads
...
function generateMockFailureDetails(failureType) {
  ...
  const errorMessages = {
    ...
    [FailureType.THREADS_CONTROLLABLE]: [
      // Mix of controllable and non-controllable errors - will be filtered in frontend
      "USER_NOT_MAPPED",
      "Phone number +1234567890 is not linked to a registered campaign and is sending a message to a phone number in the US region",
      "Phone number +9876543210 is not linked to a registered campaign and is sending a message to a phone number in the US region",
      "USER_NOT_MAPPED - Campaign registration required",
      "[404] during [POST] to https://api.example.com/messages",
      "Network timeout occurred",
      "Service temporarily unavailable",
      "Carrier rejected message",
      "Destination unreachable",
      "Message blocked by carrier filter"
    ],
  ...
  let eventMessage = ""
  if (
    failureType.includes("TOTAL") ||
    failureType.includes("CONTROLLABLE") ||
    failureType.includes("NON_CONTROLLABLE")
  ) {
    // For TOTAL, CONTROLLABLE, and NON_CONTROLLABLE types, we'll create a mix of API and DELIVERY failures
